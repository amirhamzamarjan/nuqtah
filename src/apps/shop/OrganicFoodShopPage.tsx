...
export const OrganicFoodShopPage: React.FC = () => {
  ...
  useEffect(() => {
    setTheme('default');
    const update = () => {
      setProducts(store.getProducts({ departmentId: 'organic-food' }));
      setCategories(store.getCategories('organic-food'));
    };
    update();
    ...
    <div className="min-h-screen bg-[#F2ECE4] text-[#241F1B] transition-colors duration-300">
      ...
      <section className="py-6 border-b border-[#322C26]/10 bg-[#FAF6F0]/90 sticky top-20 z-30 backdrop-blur-md shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center space-x-2 overflow-x-auto py-1">
          ...
          >
            All Essentials ({products.length})
          </button>
          {categories.map((cat) => (
            <button
              key={cat.id}
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {filtered.length === 0 ? (
          <div className="py-20 text-center space-y-3 surface-card bg-[#FDFBF7] rounded-sm border border-[#322C26]/10 p-12 shadow-sm">
            <p className="text-lg font-serif text-[#241F1B]">No organic essentials currently listed in this category.</p>
            <p className="text-xs text-[#5C5247] max-w-sm mx-auto font-light leading-relaxed">
              Publish new raw honeys and natural foods directly from the admin panel.
            </p>
          </div>
        ) : (
