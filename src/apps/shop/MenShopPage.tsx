...
export const MenShopPage: React.FC = () => {
  ...
  useEffect(() => {
    setTheme('default');
    const update = () => {
      setProducts(store.getProducts({ departmentId: 'men' }));
      setCategories(store.getCategories('men'));
    };
    update();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap items-center justify-between gap-4">
          ...
          {availableSizes.length > 0 && (
            <select
              ...
              className="bg-[#FAF6F0] text-[#241F1B] border border-[#322C26]/20 rounded-sm px-3 py-2 text-xs font-mono focus:outline-none focus:border-[#A6854F]"
            >
